...
import com.example.earthquake.Earth;
    ...
    public static ArrayList<Earth> extractEarthquakes() {
        ...
        try {
            ...
            for(int i=0;i<features.length();i++){
                JSONObject eartquakeObj = features.getJSONObject(i);
                JSONObject propertiees = eartquakeObj.getJSONObject("properties");
                Double magnitude = propertiees.getDouble("mag");
                String place = propertiees.getString("place");
                long timeLn = propertiees.getLong("time");
               // Date dateObj = new Date(timeLn);
               // SimpleDateFormat dateFormatter = new SimpleDateFormat("MMM DD, yyyy");
               // String dateToDisplay = dateFormatter.format(dateObj);
                earthquakes.add(new Earth(magnitude,place,timeLn));
